...
二维医学图像分割需要为每个像素预测类别，而不是只给整张图一个标签。COPLENet 使用编码器提取多尺度胸片特征，再通过解码路径恢复空间分辨率，输出肺野的二值掩膜；全监督训练意味着每张训练图像都配有像素级肺野标注。

Dice 衡量预测区域与真实区域的重叠程度，越接近 1 越好；ASSD 计算两条边界之间的平均对称表面距离，越低越好。两者分别反映区域一致性和边界精度。
