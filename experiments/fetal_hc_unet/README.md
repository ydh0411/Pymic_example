...
UNet 通过下采样编码器提取超声图像的上下文特征，再由上采样解码器恢复像素级头部轮廓；同尺度跳跃连接把浅层边缘信息直接传给解码器，适合边界定位要求较高的医学分割。模型输出前景概率图，经阈值化后得到胎儿头部二值掩膜。

Dice 用于衡量预测头部区域与人工标注的重叠，ASSD 用于衡量两条轮廓之间的平均距离。超声图像常含散斑噪声和弱边界，因此同时观察区域重叠与边界误差比单一指标更完整。
