...
本实验把结核筛查建模为二分类问题：ResNet18 将胸片逐层编码成高级图像特征，最后的全连接层输出“正常”和“结核异常”的概率。ImageNet 预训练为网络提供通用边缘、纹理和形状表示，再通过全参数微调适应胸片的灰度结构与病灶模式。

ROC 曲线展示不同概率阈值下灵敏度与假阳性率的权衡，AUC 越接近 1，说明模型对两类胸片的排序区分能力越强。Accuracy 反映阈值为 0.5 时的整体正确率；灵敏度关注结核病例检出，特异度关注正常病例排除。
...
![CHNCXR classification summary](figures/fig_chncxr_summary.png)

三联图依次给出测试集 ROC、训练/验证 accuracy 变化，以及两类样本的结核预测概率分布。单独的 ROC 图仍保存在 [`figures/fig_chncxr_roc.png`](figures/fig_chncxr_roc.png)。
...
脚本读取固定测试集、预测概率和训练日志，同时生成 ROC 与综合三联图；每张图均导出 300 DPI PNG 和矢量 PDF。
