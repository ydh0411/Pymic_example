...
| [ACDC UNet2D](experiments/acdc_unet/README.md) | 心脏 MRI 多结构二维分割 | NVIDIA CUDA | 完成 | Mean Dice 91.44% | LV Dice 94.27% |
| [PROMISE12 UNet2D5](experiments/prostate_unet2d5/README.md) | 前列腺 MRI 三维分割 | NVIDIA CUDA | 完成（含失败病例分析） | Dice 77.00% | ASSD 3.482 mm |
...
ACDC UNet2D 对右心室、心肌和左心室进行四分类分割，在 40 个独立测试体积上取得 91.44% 三类平均 Dice。

![ACDC per-structure Dice](experiments/acdc_unet/figures/fig_acdc_structure_dice.png)

PROMISE12 UNet2D5 在 10 例测试集上取得 77.00% Dice；其中 1 例预测失败，其余 9 例平均 Dice 为 85.55%。仓库保留完整结果并单独分析失败病例。

![PROMISE12 case metrics](experiments/prostate_unet2d5/figures/fig_prostate_case_metrics.png)
...
| ACDC UNet2D | Windows 11 | 3.10.19 | 2.10.0+cu130 | NVIDIA CUDA |
| PROMISE12 UNet2D5 | Windows 11 | 3.10.19 | 2.10.0+cu130 | NVIDIA CUDA |
...
- ACDC 使用预处理后的短轴心脏 MRI 体积及 RV、Myo、LV 多类别标注。
- PROMISE12 使用预处理后的三维 T2 MRI 和前列腺二值标注。
...
- 当前已验证 PyMIC 分类、二维及三维全监督分割流程；半监督、弱监督及噪声标签学习等模式仍需继续验证。
